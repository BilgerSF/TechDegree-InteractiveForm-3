...
$('form').attr('method',''); // I removed the default post method so that the page won't crash when refreshing
...
function validate(field){
    ...
    $('option[value="credit card"]').attr('selected',true) // set credit payment method as default
    //Default
    let selected;
    $('#paypal').hide();
    $('#bitcoin').hide();
    ...
    $('form').on('submit', (e) =>{ // submit handler on form element
...
// validaiton that applies when the payment method is not 'credit card'
    let isSelected = $('option[value = "credit card"]').is(':selected') // check if this option is selected
    
    if(    (isSelected===false) && (validate('email') === true) && (checkedBoxes > 0) ){
        submit =true;
    }
